...
add_action( 'mncf7_init', 'mncf7_add_shortcode_checkbox' );

function mncf7_add_shortcode_checkbox() {
	mncf7_add_shortcode( array( 'checkbox', 'checkbox*', 'radio' ),
		'mncf7_checkbox_shortcode_handler', true );
}

function mncf7_checkbox_shortcode_handler( $tag ) {
	...
	if ( $exclusive )
		$class .= ' mncf7-exclusive-checkbox';
	...
	$hangover = mncf7_get_hangover( $tag->name, $multiple ? array() : '' );

	foreach ( $values as $key => $value ) {
		$class = 'mncf7-list-item';
		...
		$item_atts = mncf7_format_atts( $item_atts );

		if ( $label_first ) { // put label first, input last
			$item = sprintf(
				'<span class="mncf7-list-item-label">%1$s</span>&nbsp;<input %2$s />',
				esc_html( $label ), $item_atts );
		} else {
			$item = sprintf(
				'<input %2$s />&nbsp;<span class="mncf7-list-item-label">%1$s</span>',
				esc_html( $label ), $item_atts );
		}
		...
		if ( count( $values ) == $count ) { // last round
			...
			if ( $free_text ) {
				$free_text_name = sprintf(
					'_mncf7_%1$s_free_text_%2$s', $tag->basetype, $tag->name );

				$free_text_atts = array(
					'name' => $free_text_name,
					'class' => 'mncf7-free-text',
					'tabindex' => $tabindex ? $tabindex : '' );

				if ( mncf7_is_posted() && isset( $_POST[$free_text_name] ) ) {
					$free_text_atts['value'] = mn_unslash(
						$_POST[$free_text_name] );
				}

				$free_text_atts = mncf7_format_atts( $free_text_atts );
	...
	$html = sprintf(
		'<span class="mncf7-form-control-wrap %1$s"><span %2$s>%3$s</span>%4$s</span>',
...
add_filter( 'mncf7_validate_checkbox', 'mncf7_checkbox_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_checkbox*', 'mncf7_checkbox_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_radio', 'mncf7_checkbox_validation_filter', 10, 2 );

function mncf7_checkbox_validation_filter( $result, $tag ) {
	$tag = new MNCF7_Shortcode( $tag );
...
add_filter( 'mncf7_posted_data', 'mncf7_checkbox_posted_data' );

function mncf7_checkbox_posted_data( $posted_data ) {
	$tags = mncf7_scan_shortcode(
...
add_action( 'mncf7_admin_init',
	'mncf7_add_tag_generator_checkbox_and_radio', 30 );

function mncf7_add_tag_generator_checkbox_and_radio() {
	$tag_generator = MNCF7_TagGenerator::get_instance();
	$tag_generator->add( 'checkbox', __( 'checkboxes', 'contact-form-7' ),
		'mncf7_tag_generator_checkbox' );
	$tag_generator->add( 'radio', __( 'radio buttons', 'contact-form-7' ),
		'mncf7_tag_generator_checkbox' );
}

function mncf7_tag_generator_checkbox( $contact_form, $args = '' ) {
	...
	$desc_link = mncf7_link( __( 'http://contactform7.com/checkboxes-radio-buttons-and-menus/', 'contact-form-7' ), __( 'Checkboxes, Radio Buttons and Menus', 'contact-form-7' ) );

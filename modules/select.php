...
add_action( 'mncf7_init', 'mncf7_add_shortcode_select' );

function mncf7_add_shortcode_select() {
	mncf7_add_shortcode( array( 'select', 'select*' ),
		'mncf7_select_shortcode_handler', true );
}

function mncf7_select_shortcode_handler( $tag ) {
	...
	$html = '';
	$hangover = mncf7_get_hangover( $tag->name );
	...
	$html = sprintf(
		'<span class="mncf7-form-control-wrap %1$s"><select %2$s>%3$s</select>%4$s</span>',
...
add_filter( 'mncf7_validate_select', 'mncf7_select_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_select*', 'mncf7_select_validation_filter', 10, 2 );

function mncf7_select_validation_filter( $result, $tag ) {
	$tag = new MNCF7_Shortcode( $tag );
...
add_action( 'mncf7_admin_init', 'mncf7_add_tag_generator_menu', 25 );

function mncf7_add_tag_generator_menu() {
	$tag_generator = MNCF7_TagGenerator::get_instance();
	$tag_generator->add( 'menu', __( 'drop-down menu', 'contact-form-7' ),
		'mncf7_tag_generator_menu' );
}

function mncf7_tag_generator_menu( $contact_form, $args = '' ) {

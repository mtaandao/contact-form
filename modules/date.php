...
add_action( 'mncf7_init', 'mncf7_add_shortcode_date' );

function mncf7_add_shortcode_date() {
	mncf7_add_shortcode( array( 'date', 'date*' ),
		'mncf7_date_shortcode_handler', true );
}

function mncf7_date_shortcode_handler( $tag ) {
...
add_filter( 'mncf7_validate_date', 'mncf7_date_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_date*', 'mncf7_date_validation_filter', 10, 2 );

function mncf7_date_validation_filter( $result, $tag ) {
	$tag = new MNCF7_Shortcode( $tag );
	...
	} elseif ( '' != $value && ! mncf7_is_date( $value ) ) {
		$result->invalidate( $tag, mncf7_get_message( 'invalid_date' ) );
	} elseif ( '' != $value && ! empty( $min ) && $value < $min ) {
		$result->invalidate( $tag, mncf7_get_message( 'date_too_early' ) );
	} elseif ( '' != $value && ! empty( $max ) && $max < $value ) {
		$result->invalidate( $tag, mncf7_get_message( 'date_too_late' ) );
	}
...
add_filter( 'mncf7_messages', 'mncf7_date_messages' );

function mncf7_date_messages( $messages ) {
...
add_action( 'mncf7_admin_init', 'mncf7_add_tag_generator_date', 19 );

function mncf7_add_tag_generator_date() {
	$tag_generator = MNCF7_TagGenerator::get_instance();
	$tag_generator->add( 'date', __( 'date', 'contact-form-7' ),
		'mncf7_tag_generator_date' );
}

function mncf7_tag_generator_date( $contact_form, $args = '' ) {
	...
	$desc_link = mncf7_link( __( 'http://contactform7.com/date-field/', 'contact-form-7' ), __( 'Date Field', 'contact-form-7' ) );

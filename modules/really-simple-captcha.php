...
add_action( 'mncf7_init', 'mncf7_add_shortcode_captcha' );

function mncf7_add_shortcode_captcha() {
	mncf7_add_shortcode( array( 'captchac', 'captchar' ),
		'mncf7_captcha_shortcode_handler', true );
}

function mncf7_captcha_shortcode_handler( $tag ) {
	...
	if ( 'captchac' == $tag->type ) { // CAPTCHA-Challenge (image)
		...
		$op = array_merge( $op, mncf7_captchac_options( $tag->options ) );

		if ( ! $filename = mncf7_generate_captcha( $op ) ) {
			return '';
		}
		...
		$atts['alt'] = 'captcha';
		$atts['src'] = mncf7_captcha_url( $filename );
		...
		$html = sprintf(
			'<input type="hidden" name="_mncf7_captcha_challenge_%1$s" value="%2$s" /><img %3$s />',
		...
		$html = sprintf(
			'<span class="mncf7-form-control-wrap %1$s"><input %2$s />%3$s</span>',
...
add_filter( 'mncf7_validate_captchar', 'mncf7_captcha_validation_filter', 10, 2 );

function mncf7_captcha_validation_filter( $result, $tag ) {
	$tag = new MNCF7_Shortcode( $tag );

	$type = $tag->type;
	$name = $tag->name;

	$captchac = '_mncf7_captcha_challenge_' . $name;

	$prefix = isset( $_POST[$captchac] ) ? (string) $_POST[$captchac] : '';
	$response = isset( $_POST[$name] ) ? (string) $_POST[$name] : '';
	$response = mncf7_canonicalize( $response );

	if ( 0 == strlen( $prefix ) || ! mncf7_check_captcha( $prefix, $response ) ) {
		$result->invalidate( $tag, mncf7_get_message( 'captcha_not_match' ) );
	}

	if ( 0 != strlen( $prefix ) ) {
		mncf7_remove_captcha( $prefix );
	}
...
add_filter( 'mncf7_ajax_onload', 'mncf7_captcha_ajax_refill' );
add_filter( 'mncf7_ajax_json_echo', 'mncf7_captcha_ajax_refill' );

function mncf7_captcha_ajax_refill( $items ) {
	if ( ! is_array( $items ) )
		return $items;

	$fes = mncf7_scan_shortcode( array( 'type' => 'captchac' ) );
	...
	foreach ( $fes as $fe ) {
		...
		$op = mncf7_captchac_options( $options );
		if ( $filename = mncf7_generate_captcha( $op ) ) {
			$captcha_url = mncf7_captcha_url( $filename );
...
add_filter( 'mncf7_messages', 'mncf7_captcha_messages' );

function mncf7_captcha_messages( $messages ) {
...
add_action( 'mncf7_admin_init', 'mncf7_add_tag_generator_captcha', 46 );

function mncf7_add_tag_generator_captcha() {
	if ( ! mncf7_use_really_simple_captcha() ) {
		return;
	}
	...
	$tag_generator->add( 'captcha',
		...
		'mncf7_tag_generator_captcha' );
}

function mncf7_tag_generator_captcha( $contact_form, $args = '' ) {
...
<fieldset>
<legend><?php echo sprintf( esc_html( __( "To use CAPTCHA, you first need to install and activate %s plugin.", 'contact-form-7' ) ), mncf7_link( 'http://wordpress.org/extend/plugins/really-simple-captcha/', 'Really Simple CAPTCHA' ) ); ?></legend>
</fieldset>
</div>
<?php
	...
	$desc_link = mncf7_link( __( 'http://contactform7.com/captcha/', 'contact-form-7' ), __( 'CAPTCHA', 'contact-form-7' ) );
...
add_action( 'mncf7_admin_warnings', 'mncf7_captcha_display_warning_message' );

function mncf7_captcha_display_warning_message() {
	...
	$uploads_dir = mncf7_captcha_tmp_dir();
	mncf7_init_captcha();

	if ( ! is_dir( $uploads_dir ) || ! mn_is_writable( $uploads_dir ) ) {
...
function mncf7_init_captcha() {
	...
	$dir = trailingslashit( mncf7_captcha_tmp_dir() );
...
function mncf7_captcha_tmp_dir() {
	if ( defined( 'MNCF7_CAPTCHA_TMP_DIR' ) )
		return MNCF7_CAPTCHA_TMP_DIR;
	else
		return mncf7_upload_dir( 'dir' ) . '/mncf7_captcha';
}

function mncf7_captcha_tmp_url() {
	if ( defined( 'MNCF7_CAPTCHA_TMP_URL' ) )
		return MNCF7_CAPTCHA_TMP_URL;
	else
		return mncf7_upload_dir( 'url' ) . '/mncf7_captcha';
}

function mncf7_captcha_url( $filename ) {
	$url = trailingslashit( mncf7_captcha_tmp_url() ) . $filename;

	if ( is_ssl() && 'http:' == substr( $url, 0, 5 ) ) {
		$url = 'https:' . substr( $url, 5 );
	}

	return apply_filters( 'mncf7_captcha_url', esc_url_raw( $url ) );
}

function mncf7_generate_captcha( $options = null ) {
	if ( ! $captcha = mncf7_init_captcha() ) {
		return false;
	}

	if ( ! is_dir( $captcha->tmp_dir ) || ! mn_is_writable( $captcha->tmp_dir ) )
		return false;
...
function mncf7_check_captcha( $prefix, $response ) {
	if ( ! $captcha = mncf7_init_captcha() ) {
		return false;
	}
...
function mncf7_remove_captcha( $prefix ) {
	if ( ! $captcha = mncf7_init_captcha() ) {
		return false;
	}

	if ( preg_match( '/[^0-9]/', $prefix ) ) // Contact Form 7 generates $prefix with mn_rand()
		return false;

	$captcha->remove( $prefix );
}

add_action( 'template_redirect', 'mncf7_cleanup_captcha_files', 20 );

function mncf7_cleanup_captcha_files() {
	if ( ! $captcha = mncf7_init_captcha() ) {
		return false;
	}
	...
	if ( ! is_dir( $dir ) || ! is_readable( $dir ) || ! mn_is_writable( $dir ) )
		return false;
...
function mncf7_captchac_options( $options ) {
	if ( ! is_array( $options ) )
		return array();

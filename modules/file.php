...
add_action( 'mncf7_init', 'mncf7_add_shortcode_file' );

function mncf7_add_shortcode_file() {
	mncf7_add_shortcode( array( 'file', 'file*' ),
		'mncf7_file_shortcode_handler', true );
}

function mncf7_file_shortcode_handler( $tag ) {
...
add_filter( 'mncf7_form_enctype', 'mncf7_file_form_enctype_filter' );

function mncf7_file_form_enctype_filter( $enctype ) {
	$multipart = (bool) mncf7_scan_shortcode( array( 'type' => array( 'file', 'file*' ) ) );
...
add_filter( 'mncf7_validate_file', 'mncf7_file_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_file*', 'mncf7_file_validation_filter', 10, 2 );

function mncf7_file_validation_filter( $result, $tag ) {
	$tag = new MNCF7_Shortcode( $tag );
	...
	if ( $file['error'] && UPLOAD_ERR_NO_FILE != $file['error'] ) {
		$result->invalidate( $tag, mncf7_get_message( 'upload_failed_php_error' ) );
		return $result;
	}

	if ( empty( $file['tmp_name'] ) && $tag->is_required() ) {
		$result->invalidate( $tag, mncf7_get_message( 'invalid_required' ) );
		return $result;
	}
	...
	if ( ! preg_match( $file_type_pattern, $file['name'] ) ) {
		$result->invalidate( $tag, mncf7_get_message( 'upload_file_type_invalid' ) );
		return $result;
	}
	...
	if ( $file['size'] > $allowed_size ) {
		$result->invalidate( $tag, mncf7_get_message( 'upload_file_too_large' ) );
		return $result;
	}

	mncf7_init_uploads(); // Confirm upload dir
	$uploads_dir = mncf7_upload_tmp_dir();
	$uploads_dir = mncf7_maybe_add_random_dir( $uploads_dir );

	$filename = $file['name'];
	$filename = mncf7_canonicalize( $filename );
	$filename = sanitize_file_name( $filename );
	$filename = mncf7_antiscript_file_name( $filename );
	$filename = mn_unique_filename( $uploads_dir, $filename );

	$new_file = trailingslashit( $uploads_dir ) . $filename;

	if ( false === @move_uploaded_file( $file['tmp_name'], $new_file ) ) {
		$result->invalidate( $tag, mncf7_get_message( 'upload_failed' ) );
		return $result;
	}
...
add_filter( 'mncf7_messages', 'mncf7_file_messages' );

function mncf7_file_messages( $messages ) {
...
add_action( 'mncf7_admin_init', 'mncf7_add_tag_generator_file', 50 );

function mncf7_add_tag_generator_file() {
	$tag_generator = MNCF7_TagGenerator::get_instance();
	$tag_generator->add( 'file', __( 'file', 'contact-form-7' ),
		'mncf7_tag_generator_file' );
}

function mncf7_tag_generator_file( $contact_form, $args = '' ) {
	...
	$desc_link = mncf7_link( __( 'http://contactform7.com/file-uploading-and-attachment/', 'contact-form-7' ), __( 'File Uploading and Attachment', 'contact-form-7' ) );
...
add_action( 'mncf7_admin_warnings', 'mncf7_file_display_warning_message' );

function mncf7_file_display_warning_message() {
...
function mncf7_init_uploads() {
	$dir = mncf7_upload_tmp_dir();
	mn_mkdir_p( $dir );
...
function mncf7_maybe_add_random_dir( $dir ) {
	do {
...
function mncf7_upload_tmp_dir() {
	if ( defined( 'MNCF7_UPLOADS_TMP_DIR' ) )
		return MNCF7_UPLOADS_TMP_DIR;
	else
		return mncf7_upload_dir( 'dir' ) . '/mncf7_uploads';
}

add_action( 'template_redirect', 'mncf7_cleanup_upload_files', 20 );

function mncf7_cleanup_upload_files( $seconds = 60, $max = 100 ) {
	if ( is_admin() || 'GET' != $_SERVER['REQUEST_METHOD']
	|| is_robots() || is_feed() || is_trackback() ) {
		return;
	}

	$dir = trailingslashit( mncf7_upload_tmp_dir() );
	...
	if ( $handle = @opendir( $dir ) ) {
		while ( false !== ( $file = readdir( $handle ) ) ) {
			...
			mncf7_rmdir_p( path_join( $dir, $file ) );
			$count += 1;

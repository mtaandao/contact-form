...
add_action( 'mncf7_init', 'mncf7_add_shortcode_text' );

function mncf7_add_shortcode_text() {
	mncf7_add_shortcode(
		array( 'text', 'text*', 'email', 'email*', 'url', 'url*', 'tel', 'tel*' ),
		'mncf7_text_shortcode_handler', true );
}

function mncf7_text_shortcode_handler( $tag ) {
	...
	$class = mncf7_form_controls_class( $tag->type, 'mncf7-text' );

	if ( in_array( $tag->basetype, array( 'email', 'url', 'tel' ) ) ) {
		$class .= ' mncf7-validates-as-' . $tag->basetype;
	}
	...
	$value = mncf7_get_hangover( $tag->name, $value );

	$atts['value'] = $value;

	if ( mncf7_support_html5() ) {
...
add_filter( 'mncf7_validate_text', 'mncf7_text_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_text*', 'mncf7_text_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_email', 'mncf7_text_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_email*', 'mncf7_text_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_url', 'mncf7_text_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_url*', 'mncf7_text_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_tel', 'mncf7_text_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_tel*', 'mncf7_text_validation_filter', 10, 2 );

function mncf7_text_validation_filter( $result, $tag ) {
	$tag = new MNCF7_Shortcode( $tag );

	$name = $tag->name;

	$value = isset( $_POST[$name] )
		? trim( mn_unslash( strtr( (string) $_POST[$name], "\n", " " ) ) )
		: '';
	...
	if ( 'email' == $tag->basetype ) {
		if ( $tag->is_required() && '' == $value ) {
			$result->invalidate( $tag, mncf7_get_message( 'invalid_required' ) );
		} elseif ( '' != $value && ! mncf7_is_email( $value ) ) {
			$result->invalidate( $tag, mncf7_get_message( 'invalid_email' ) );
		}
	}
	...
	if ( 'url' == $tag->basetype ) {
		if ( $tag->is_required() && '' == $value ) {
			$result->invalidate( $tag, mncf7_get_message( 'invalid_required' ) );
		} elseif ( '' != $value && ! mncf7_is_url( $value ) ) {
			$result->invalidate( $tag, mncf7_get_message( 'invalid_url' ) );
		}
	}
	...
	if ( 'tel' == $tag->basetype ) {
		if ( $tag->is_required() && '' == $value ) {
			$result->invalidate( $tag, mncf7_get_message( 'invalid_required' ) );
		} elseif ( '' != $value && ! mncf7_is_tel( $value ) ) {
			$result->invalidate( $tag, mncf7_get_message( 'invalid_tel' ) );
		}
	}
	...
	if ( ! empty( $value ) ) {
		...
		$code_units = mncf7_count_code_units( stripslashes( $value ) );

		if ( false !== $code_units ) {
			if ( $maxlength && $maxlength < $code_units ) {
				$result->invalidate( $tag, mncf7_get_message( 'invalid_too_long' ) );
			} elseif ( $minlength && $code_units < $minlength ) {
				$result->invalidate( $tag, mncf7_get_message( 'invalid_too_short' ) );
			}
		}
	}
...
add_filter( 'mncf7_messages', 'mncf7_text_messages' );

function mncf7_text_messages( $messages ) {
...
add_action( 'mncf7_admin_init', 'mncf7_add_tag_generator_text', 15 );

function mncf7_add_tag_generator_text() {
	$tag_generator = MNCF7_TagGenerator::get_instance();
	$tag_generator->add( 'text', __( 'text', 'contact-form-7' ),
		'mncf7_tag_generator_text' );
...
function mncf7_tag_generator_text( $contact_form, $args = '' ) {
	...
	$desc_link = mncf7_link( __( 'http://contactform7.com/text-fields/', 'contact-form-7' ), __( 'Text Fields', 'contact-form-7' ) );

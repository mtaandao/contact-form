...
add_action( 'mncf7_init', 'mncf7_add_shortcode_number' );

function mncf7_add_shortcode_number() {
	mncf7_add_shortcode( array( 'number', 'number*', 'range', 'range*' ),
		'mncf7_number_shortcode_handler', true );
}

function mncf7_number_shortcode_handler( $tag ) {
	...
	$class .= ' mncf7-validates-as-number';
...
add_filter( 'mncf7_validate_number', 'mncf7_number_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_number*', 'mncf7_number_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_range', 'mncf7_number_validation_filter', 10, 2 );
add_filter( 'mncf7_validate_range*', 'mncf7_number_validation_filter', 10, 2 );

function mncf7_number_validation_filter( $result, $tag ) {
	$tag = new MNCF7_Shortcode( $tag );
	...
	} elseif ( '' != $value && ! mncf7_is_number( $value ) ) {
		$result->invalidate( $tag, mncf7_get_message( 'invalid_number' ) );
	} elseif ( '' != $value && '' != $min && (float) $value < (float) $min ) {
		$result->invalidate( $tag, mncf7_get_message( 'number_too_small' ) );
	} elseif ( '' != $value && '' != $max && (float) $max < (float) $value ) {
		$result->invalidate( $tag, mncf7_get_message( 'number_too_large' ) );
	}
...
add_filter( 'mncf7_messages', 'mncf7_number_messages' );

function mncf7_number_messages( $messages ) {
...
add_action( 'mncf7_admin_init', 'mncf7_add_tag_generator_number', 18 );

function mncf7_add_tag_generator_number() {
	$tag_generator = MNCF7_TagGenerator::get_instance();
	$tag_generator->add( 'number', __( 'number', 'contact-form-7' ),
		'mncf7_tag_generator_number' );
}

function mncf7_tag_generator_number( $contact_form, $args = '' ) {
	...
	$desc_link = mncf7_link( __( 'http://contactform7.com/number-fields/', 'contact-form-7' ), __( 'Number Fields', 'contact-form-7' ) );

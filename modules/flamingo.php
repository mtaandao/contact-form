...
add_action( 'mncf7_submit', 'mncf7_flamingo_submit', 10, 2 );

function mncf7_flamingo_submit( $contactform, $result ) {
	...
	$cases = (array) apply_filters( 'mncf7_flamingo_submit_if',
	...
	$email = mncf7_flamingo_get_value( 'email', $contactform );
	$name = mncf7_flamingo_get_value( 'name', $contactform );
	$subject = mncf7_flamingo_get_value( 'subject', $contactform );
	...
	foreach ( $special_mail_tags as $smt ) {
		$meta[$smt] = apply_filters( 'mncf7_special_mail_tags',
			'', '_' . $smt, false );
	}
	...
	$channel_id = mncf7_flamingo_add_channel(
	...
	if ( $channel_id ) {
		...
		if ( ! $channel || is_mn_error( $channel ) ) {
...
function mncf7_flamingo_get_value( $field, $contactform ) {
	...
	if ( in_array( $field, array( 'email', 'name', 'subject' ) ) ) {
		...
		if ( empty( $templates[0] ) ) {
			...
			$template = trim( mncf7_strip_quote( $templates[0] ) );
		}

		$value = mncf7_mail_replace_tags( $template );
	}

	$value = apply_filters( 'mncf7_flamingo_get_value', $value,
		$field, $contactform );

	return $value;
}

function mncf7_flamingo_add_channel( $slug, $name = '' ) {
	...
	if ( ! $parent ) {
		$parent = mn_insert_term( __( 'Contact Form 7', 'contact-form-7' ),
			Flamingo_Inbound_Message::channel_taxonomy,
	...
	if ( ! $channel ) {
		$channel = mn_insert_term( $name,
			Flamingo_Inbound_Message::channel_taxonomy,
			array( 'slug' => $slug, 'parent' => $parent ) );

		if ( is_mn_error( $channel ) ) {
			return false;
		}
	}

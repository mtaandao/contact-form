...
class MNCF7_RECAPTCHA extends MNCF7_Service {
	...
	private function __construct() {
		$this->sitekeys = MNCF7::get_option( 'recaptcha' );
	}
	...
	public function verify( $response_token ) {
		...
		$response = mn_safe_remote_post( $url, array(
			'body' => array(
				'secret' => $secret,
	...
	private function menu_page_url( $args = '' ) {
		...
		$url = menu_page_url( 'mncf7-integration', false );
	...
	public function load( $action = '' ) {
		if ( 'setup' == $action ) {
			if ( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
				check_admin_referer( 'mncf7-recaptcha-setup' );

				$sitekey = isset( $_POST['sitekey'] ) ? trim( $_POST['sitekey'] ) : '';
				$secret = isset( $_POST['secret'] ) ? trim( $_POST['secret'] ) : '';

				if ( $sitekey && $secret ) {
					MNCF7::update_option( 'recaptcha', array( $sitekey => $secret ) );
					$redirect_to = $this->menu_page_url( array(
						'message' => 'success' ) );
				} elseif ( '' === $sitekey && '' === $secret ) {
					MNCF7::update_option( 'recaptcha', null );
					$redirect_to = $this->menu_page_url( array(
						'message' => 'success' ) );
				} else {
					$redirect_to = $this->menu_page_url( array(
...
<tr>
	...
	<td class="code"><?php echo esc_html( mncf7_mask_password( $secret ) ); ?></td>
</tr>
</tbody>
</table>
...
<p><?php echo sprintf( esc_html( __( "For more details, see %s.", 'contact-form-7' ) ), mncf7_link( __( 'http://contactform7.com/recaptcha/', 'contact-form-7' ), __( 'reCAPTCHA', 'contact-form-7' ) ) ); ?></p>
<?php
		}
	}
...
<?php mn_nonce_field( 'mncf7-recaptcha-setup' ); ?>
<table class="form-table">
<tbody>
<tr>
...
add_action( 'mncf7_init', 'mncf7_recaptcha_register_service' );

function mncf7_recaptcha_register_service() {
	...
	$services = array(
		'recaptcha' => MNCF7_RECAPTCHA::get_instance() );
...
add_action( 'mncf7_enqueue_scripts', 'mncf7_recaptcha_enqueue_scripts' );

function mncf7_recaptcha_enqueue_scripts() {
	$url = 'https://www.google.com/recaptcha/api.js';
	$url = add_query_arg( array(
		'onload' => 'recaptchaCallback',
		'render' => 'explicit' ), $url );

	mn_register_script( 'google-recaptcha', $url, array(), '2.0', true );
}

add_action( 'mn_footer', 'mncf7_recaptcha_callback_script' );

function mncf7_recaptcha_callback_script() {
	if ( ! mn_script_is( 'google-recaptcha', 'enqueued' ) ) {
		return;
	}
...
add_action( 'mncf7_init', 'mncf7_recaptcha_add_shortcode_recaptcha' );

function mncf7_recaptcha_add_shortcode_recaptcha() {
	$recaptcha = MNCF7_RECAPTCHA::get_instance();

	if ( $recaptcha->is_active() ) {
		mncf7_add_shortcode( 'recaptcha', 'mncf7_recaptcha_shortcode_handler' );
	}
}

function mncf7_recaptcha_shortcode_handler( $tag ) {
	mn_enqueue_script( 'google-recaptcha' );
	...
	$atts['class'] = $tag->get_class_option(
		mncf7_form_controls_class( $tag->type, 'g-recaptcha' ) );
	$atts['id'] = $tag->get_id_option();

	$html = sprintf( '<div %1$s></div>', mncf7_format_atts( $atts ) );
	$html .= mncf7_recaptcha_noscript(
		array( 'sitekey' => $atts['data-sitekey'] ) );
	$html = sprintf( '<div class="mncf7-form-control-wrap">%s</div>', $html );

	return $html;
}

function mncf7_recaptcha_noscript( $args = '' ) {
	$args = mn_parse_args( $args, array(
		'sitekey' => '' ) );
...
add_filter( 'mncf7_spam', 'mncf7_recaptcha_check_with_google', 9 );

function mncf7_recaptcha_check_with_google( $spam ) {
	if ( $spam ) {
		return $spam;
	}
	...
	$response_token = mncf7_recaptcha_response();
	$spam = ! $recaptcha->verify( $response_token );

	return $spam;
}

add_action( 'mncf7_admin_init', 'mncf7_add_tag_generator_recaptcha', 45 );

function mncf7_add_tag_generator_recaptcha() {
	$tag_generator = MNCF7_TagGenerator::get_instance();
	$tag_generator->add( 'recaptcha', __( 'reCAPTCHA', 'contact-form-7' ),
		'mncf7_tag_generator_recaptcha', array( 'nameless' => 1 ) );
}

function mncf7_tag_generator_recaptcha( $contact_form, $args = '' ) {
	$args = mn_parse_args( $args, array() );
...
<fieldset>
<legend><?php echo sprintf( esc_html( __( "To use reCAPTCHA, first you need to install an API key pair. For more details, see %s.", 'contact-form-7' ) ), mncf7_link( __( 'http://contactform7.com/recaptcha/', 'contact-form-7' ), __( 'reCAPTCHA', 'contact-form-7' ) ) ); ?></legend>
</fieldset>
</div>
<?php
	...
	$desc_link = mncf7_link( __( 'http://contactform7.com/recaptcha/', 'contact-form-7' ), __( 'reCAPTCHA', 'contact-form-7' ) );
...
function mncf7_recaptcha_response() {

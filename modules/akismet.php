...
add_filter( 'mncf7_spam', 'mncf7_akismet' );

function mncf7_akismet( $spam ) {
	if ( $spam ) {
		return $spam;
	}

	if ( ! mncf7_akismet_is_available() ) {
		return false;
	}

	if ( ! $params = mncf7_akismet_submitted_params() ) {
		return false;
	}
	...
	return mncf7_akismet_comment_check( $c );
}

function mncf7_akismet_is_available() {
...
function mncf7_akismet_submitted_params() {
	$params = array(
		'author' => '',
		'author_email' => '',
		'author_url' => '',
	...
	foreach ( (array) $_POST as $key => $val ) {
		if ( '_mncf7' == substr( $key, 0, 6 ) || '_mnnonce' == $key ) {
			continue;
		}

		if ( is_array( $val ) ) {
			$val = implode( ', ', mncf7_array_flatten( $val ) );
		}
		...
		if ( $tags = mncf7_scan_shortcode( array( 'name' => $key ) ) ) {
			$tag = $tags[0];
...
function mncf7_akismet_comment_check( $comment ) {
	global $akismet_api_host, $akismet_api_port;

	$spam = false;
	$query_string = mncf7_build_query( $comment );
	...
	return apply_filters( 'mncf7_akismet_comment_check', $spam, $comment );
}

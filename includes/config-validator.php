...
class MNCF7_ConfigValidator {
	...
	public function __construct( MNCF7_ContactForm $contact_form ) {
	...
	public function build_message( $message, $params = '' ) {
		$params = mn_parse_args( $params, array() );
	...
	public function validate() {
		...
		do_action( 'mncf7_config_validator_validate', $this );
	...
	public function validate_mail( $template = 'mail' ) {
		...
		$components = mn_parse_args( $components, array(
			'subject' => '',
			'sender' => '',
			'recipient' => '',
			'additional_headers' => '',
		...
		$subject = new MNCF7_MailTaggedText( $subject,
			array( 'callback' => $callback ) );
		$subject = $subject->replace_tags();
		$subject = mncf7_strip_newline( $subject );
		...
		$sender = new MNCF7_MailTaggedText( $sender,
			array( 'callback' => $callback ) );
		$sender = $sender->replace_tags();
		$sender = mncf7_strip_newline( $sender );

		if ( ! mncf7_is_mailbox_list( $sender ) ) {
		...
		} elseif ( ! mncf7_is_email_in_site_domain( $sender ) ) {
			$this->add_error( sprintf( '%s.sender', $template ),
		...
		$recipient = new MNCF7_MailTaggedText( $recipient,
			array( 'callback' => $callback ) );
		$recipient = $recipient->replace_tags();
		$recipient = mncf7_strip_newline( $recipient );

		if ( ! mncf7_is_mailbox_list( $recipient ) ) {
		...
		$additional_headers = new MNCF7_MailTaggedText( $additional_headers,
			array( 'callback' => $callback ) );
		...
		foreach ( $additional_headers as $header ) {
			...
			&& ! mncf7_is_mailbox_list( $matches[2] ) ) {
				$this->add_error( sprintf( '%s.additional_headers', $template ),
		...
		$body = new MNCF7_MailTaggedText( $body,
			array( 'callback' => $callback ) );
	...
	public function validate_messages() {
		...
		foreach ( $messages as $key => $message ) {
			$stripped = mn_strip_all_tags( $message );
	...
	public function replace_mail_tags_with_minimum_input( $matches ) {
		...
		if ( $form_tags ) {
			$form_tag = new MNCF7_Shortcode( $form_tags[0] );
			...
			if ( $is_selectable ) {
				if ( $form_tag->pipes instanceof MNCF7_Pipes ) {
					if ( $do_not_heat ) {
				...
				if ( $last_item && mncf7_is_mailbox_list( $last_item ) ) {
			...
			$tagname = preg_replace( '/^mncf7\./', '_', $tagname ); // for back-compat

...
function mncf7_is_valid_locale( $locale ) {
	$pattern = '/^[a-z]{2,3}(?:_[a-zA-Z_]{2,})?$/';
	return (bool) preg_match( $pattern, $locale );
}

function mncf7_is_rtl( $locale = '' ) {
...
function mncf7_load_textdomain( $locale = null ) {
	global $l10n;
	...
	if ( empty( $locale ) ) {
		...
		$path = MN_PLUGIN_DIR . '/' . $domain . '/languages';

		if ( $loaded = load_textdomain( $domain, $path . '/'. $mofile ) ) {
			return $loaded;
		} else {
			$mofile = MN_LANG_DIR . '/plugins/' . $mofile;

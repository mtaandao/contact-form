...
class MNCF7_ContactForm {

	const post_type = 'mncf7_contact_form';
	...
	public static function get_template( $args = '' ) {
		...
		foreach ( $properties as $key => $value ) {
			$properties[$key] = MNCF7_ContactFormTemplate::get_default( $key );
		}

		$contact_form->properties = $properties;

		$contact_form = apply_filters( 'mncf7_contact_form_default_pack',
			$contact_form, $args );
	...
	private static function get_unit_tag( $id = 0 ) {
		...
		if ( in_the_loop() ) {
			$unit_tag = sprintf( 'mncf7-f%1$d-p%2$d-o%3$d',
				absint( $id ), get_the_ID(), $global_count );
		} else {
			$unit_tag = sprintf( 'mncf7-f%1$d-o%2$d',
	...
	private function __construct( $post = null ) {
		...
		do_action( 'mncf7_contact_form', $this );
	}

	public function __get( $name ) {
		$message = __( '<code>%1$s</code> property of a <code>MNCF7_ContactForm</code> object is <strong>no longer accessible</strong>. Use <code>%2$s</code> method instead.', 'contact-form-7' );
	...
	public function get_properties() {
		...
		$properties = mn_parse_args( $properties, array(
			'form' => '',
			'mail' => array(),
			'mail_2' => array(),
			'messages' => array(),
		...
		$properties = (array) apply_filters( 'mncf7_contact_form_properties',
			$properties, $this );
	...
	public function set_properties( $properties ) {
		...
		$properties = mn_parse_args( $properties, $defaults );
	...
	public function is_posted() {
		if ( ! MNCF7_Submission::get_instance() ) {
			return false;
		}

		if ( empty( $_POST['_mncf7_unit_tag'] ) ) {
			return false;
		}

		return $this->unit_tag == $_POST['_mncf7_unit_tag'];
	}
	...
	public function form_html( $args = '' ) {
		...
		if ( 'raw_form' == $args['output'] ) {
			return '<pre class="mncf7-raw-form"><code>'
		...
		$html = sprintf( '<div %s>', mncf7_format_atts( array(
			'role' => 'form',
			'class' => 'mncf7',
			'id' => $this->unit_tag,
			( get_option( 'html_type' ) == 'text/html' ) ? 'lang' : 'xml:lang'
				=> $lang_tag,
			'dir' => mncf7_is_rtl( $this->locale ) ? 'rtl' : 'ltr' ) ) ) . "\n";

		$html .= $this->screen_reader_response() . "\n";

		$url = mncf7_get_request_uri();
		...
		$url = apply_filters( 'mncf7_form_action_url', $url );

		$id_attr = apply_filters( 'mncf7_form_id_attr',
			preg_replace( '/[^A-Za-z0-9:._-]/', '', $args['html_id'] ) );

		$name_attr = apply_filters( 'mncf7_form_name_attr',
		...
		$class = implode( ' ', $class );
		$class = apply_filters( 'mncf7_form_class_attr', $class );

		$enctype = apply_filters( 'mncf7_form_enctype', '' );
		$autocomplete = apply_filters( 'mncf7_form_autocomplete', '' );

		$novalidate = apply_filters( 'mncf7_form_novalidate',
			mncf7_support_html5() );
		...
		$atts = array(
			...
			'method' => 'post',
			'class' => $class,
			'enctype' => mncf7_enctype_value( $enctype ),
	...
	private function form_hidden_fields() {
		$hidden_fields = array(
			'_mncf7' => $this->id,
			'_mncf7_version' => MNCF7_VERSION,
			'_mncf7_locale' => $this->locale,
			'_mncf7_unit_tag' => $this->unit_tag );

		if ( MNCF7_VERIFY_NONCE ) {
			$hidden_fields['_mnnonce'] = mncf7_create_nonce( $this->id );
		}

		$hidden_fields += (array) apply_filters(
			'mncf7_form_hidden_fields', array() );
	...
	public function form_response_output() {
		...
		if ( $this->is_posted() ) { // Post response output for non-AJAX
			...
			if ( $submission->is( 'validation_failed' ) ) {
				$class .= ' mncf7-validation-errors';
			} elseif ( $submission->is( 'spam' ) ) {
				$class .= ' mncf7-spam-blocked';
			} elseif ( $submission->is( 'mail_sent' ) ) {
				$class .= ' mncf7-mail-sent-ok';
			} elseif ( $submission->is( 'mail_failed' ) ) {
				$class .= ' mncf7-mail-sent-ng';
			}
		} else {
			$class .= ' mncf7-display-none';
		}
		...
		$output = apply_filters( 'mncf7_form_response_output',
	...
	public function validation_error( $name ) {
		...
		$error = sprintf(
			'<span role="alert" class="mncf7-not-valid-tip">%s</span>',
			esc_html( $error ) );

		return apply_filters( 'mncf7_validation_error', $error, $name, $this );
	}

	/* Form Elements */

	public function form_do_shortcode() {
		$manager = MNCF7_ShortcodeManager::get_instance();
		$form = $this->prop( 'form' );

		if ( MNCF7_AUTOP ) {
			$form = $manager->normalize_shortcode( $form );
			$form = mncf7_autop( $form );
		}
	...
	public function form_elements() {
		return apply_filters( 'mncf7_form_elements', $this->form_do_shortcode() );
	}
	...
	public function collect_mail_tags( $args = '' ) {
		...
		return apply_filters( 'mncf7_collect_mail_tags', $mailtags );
	}

	public function suggest_mail_tags( $for = 'mail' ) {
		$mail = mn_parse_args( $this->prop( $for ), array(
	...
	public function submit( $ajax = false ) {
		$submission = MNCF7_Submission::get_instance( $this );
		...
		if ( $ajax ) {
			...
			if ( ! empty( $on_submit ) ) {
				$result['scripts_on_submit'] = array_map(
					'mncf7_strip_quote', $on_submit );
			}
		}

		do_action( 'mncf7_submit', $this, $result );
	...
	public function message( $status, $filter = true ) {
		...
		if ( $filter ) {
			$message = mn_strip_all_tags( $message );
			$message = mncf7_mail_replace_tags( $message, array( 'html' => true ) );
			$message = apply_filters( 'mncf7_display_message', $message, $status );
		}
	...
	private function upgrade() {
		...
		if ( is_array( $messages ) ) {
			foreach ( mncf7_messages() as $key => $arr ) {
		...
		if ( $post_id ) {
			foreach ( $props as $prop => $value ) {
				update_post_meta( $post_id, '_' . $prop,
					mncf7_normalize_newline_deep( $value ) );
			}

			if ( mncf7_is_valid_locale( $this->locale ) ) {
				update_post_meta( $post_id, '_locale', $this->locale );
			}

			if ( $this->initial() ) {
				$this->id = $post_id;
				do_action( 'mncf7_after_create', $this );
			} else {
				do_action( 'mncf7_after_update', $this );
			}

			do_action( 'mncf7_after_save', $this );
		}
	...
	public function copy() {
		...
		return apply_filters( 'mncf7_copy', $new, $this );
	}
	...
	public function delete() {
		...
		if ( mn_delete_post( $this->id, true ) ) {
			$this->id = 0;
			return true;
		}
	...
	public function shortcode( $args = '' ) {
		...
		return apply_filters( 'mncf7_contact_form_shortcode', $shortcode, $args, $this );
	}
}

function mncf7_contact_form( $id ) {
	return MNCF7_ContactForm::get_instance( $id );
}

function mncf7_get_contact_form_by_old_id( $old_id ) {
	global $mndb;

	$q = "SELECT post_id FROM $mndb->postmeta WHERE meta_key = '_old_cf7_unit_id'"
		. $mndb->prepare( " AND meta_value = %d", $old_id );

	if ( $new_id = $mndb->get_var( $q ) ) {
		return mncf7_contact_form( $new_id );
	}
}

function mncf7_get_contact_form_by_title( $title ) {
	$page = get_page_by_title( $title, OBJECT, MNCF7_ContactForm::post_type );

	if ( $page ) {
		return mncf7_contact_form( $page->ID );
	}

	return null;
}

function mncf7_get_current_contact_form() {
	if ( $current = MNCF7_ContactForm::get_current() ) {
		return $current;
	}
}

function mncf7_is_posted() {
...
function mncf7_get_hangover( $name, $default = null ) {
	if ( ! mncf7_is_posted() ) {
		return $default;
	}
	...
	return isset( $_POST[$name] ) ? mn_unslash( $_POST[$name] ) : $default;
}

function mncf7_get_validation_error( $name ) {
	if ( ! $contact_form = mncf7_get_current_contact_form() ) {
		return '';
	}
...
function mncf7_get_message( $status ) {
	if ( ! $contact_form = mncf7_get_current_contact_form() ) {
		return '';
	}
...
function mncf7_scan_shortcode( $cond = null ) {
	if ( ! $contact_form = mncf7_get_current_contact_form() ) {
		return array();
	}
...
function mncf7_form_controls_class( $type, $default = '' ) {
	$type = trim( $type );
	$default = array_filter( explode( ' ', $default ) );

	$classes = array_merge( array( 'mncf7-form-control' ), $default );

	$typebase = rtrim( $type, '*' );
	$required = ( '*' == substr( $type, -1 ) );

	$classes[] = 'mncf7-' . $typebase;

	if ( $required ) {
		$classes[] = 'mncf7-validates-as-required';
	}
...
function mncf7_contact_form_tag_func( $atts, $content = null, $code = '' ) {
	if ( is_feed() ) {
	...
	if ( 'contact-form-7' == $code ) {
		...
		if ( ! $contact_form = mncf7_contact_form( $id ) ) {
			$contact_form = mncf7_get_contact_form_by_title( $title );
		}
		...
		$contact_form = mncf7_get_contact_form_by_old_id( $id );
	}

...
class MNCF7_ShortcodeManager {
	...
	private function do_shortcode_tag( $m ) {
		...
		if ( is_array( $attr ) ) {
			if ( is_array( $attr['options'] ) ) {
				if ( $this->shortcode_tags[$tag]['has_name'] && ! empty( $attr['options'] ) ) {
					...
					if ( ! mncf7_is_name( $scanned_tag['name'] ) )
			...
			if ( MNCF7_USE_PIPE ) {
				$pipes = new MNCF7_Pipes( $scanned_tag['raw_values'] );
		...
		$scanned_tag = apply_filters( 'mncf7_form_tag', $scanned_tag, $this->exec );
	...
	private function shortcode_parse_atts( $text ) {
		...
		if ( preg_match( $pattern, $text, $match ) ) {
			...
			if ( ! empty( $match[2] ) ) {
				...
				$atts['values'] = mncf7_strip_quote_deep( $matched_values[0] );
			}
		} else {
			$atts = $text;
		}
...
class MNCF7_Shortcode {
	...
	public function get_default_option( $default = '', $args = '' ) {
		...
		foreach ( $options as $opt ) {
			...
			if ( 'user_' == substr( $opt, 0, 5 ) && is_user_logged_in() ) {
				...
				$vals = array_map( 'mncf7_sanitize_query_var', $vals );
	...
	public function get_data_option( $args = '' ) {
		...
		return apply_filters( 'mncf7_form_tag_data_option', null, $options, $args );
	}

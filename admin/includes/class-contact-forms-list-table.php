...
class MNCF7_Contact_Form_List_Table extends MN_List_Table {
	...
	function prepare_items() {
		...
		$this->items = MNCF7_ContactForm::find( $args );

		$total_items = MNCF7_ContactForm::count();
	...
	function column_title( $item ) {
		$url = admin_url( 'admin.php?page=mncf7&post=' . absint( $item->id() ) );
		...
		&& current_user_can( 'mncf7_edit_contact_form', $item->id() ) ) {
			$config_validator = new MNCF7_ConfigValidator( $item );
		...
		if ( current_user_can( 'mncf7_edit_contact_form', $item->id() ) ) {
			$copy_link = mn_nonce_url(
				add_query_arg( array( 'action' => 'copy' ), $url ),
				'mncf7-copy-contact-form_' . absint( $item->id() ) );

(function($) {
	...
	$('form.tag-generator-panel .control-box :input').change(function(event) {
		...
		_mncf7.taggen.normalize($(this));
		_mncf7.taggen.update(form);
	});

	$('input.insert-tag').click(function(event) {
		var form = $(this).closest('form.tag-generator-panel');
		var tag = form.find('input.tag').val();
		_mncf7.taggen.insert(tag);
		tb_remove(); // close thickbox
		return false;
	});

	_mncf7.taggen.update = function($form) {
		...
		if ($.isFunction(_mncf7.taggen.update[id])) {
			return _mncf7.taggen.update[id].call(this, $form);
		}
		...
		$form.find('input.tag').each(function() {
			...
			var components = _mncf7.taggen.compose(tag_type, $form);
			$(this).val(components);
		});
	...
	_mncf7.taggen.update.captcha = function($form) {
		var captchac = _mncf7.taggen.compose('captchac', $form);
		var captchar = _mncf7.taggen.compose('captchar', $form);

		$form.find('input.tag').val(captchac + ' ' + captchar);
	};

	_mncf7.taggen.compose = function(tagType, $form) {
	...
	_mncf7.taggen.normalize = function($input) {
		var val = $input.val();
		...
		if ($input.is(':checkbox.exclusive')) {
			_mncf7.taggen.exclusiveCheckbox($input);
		}
	}

	_mncf7.taggen.exclusiveCheckbox = function($cb) {
		if ($cb.is(':checked')) {
			$cb.siblings(':checkbox.exclusive').prop('checked', false);
		}
	};

	_mncf7.taggen.insert = function(content) {
		$('textarea#mncf7-form').each(function() {
			this.focus();

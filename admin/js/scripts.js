(function($) {
	...
	$(function() {
		...
		updateWelcomePanel = function( visible ) {
			$.post( ajaxurl, {
				action: 'mncf7-update-welcome-panel',
				visible: visible,
		...
		$('input:checkbox.toggle-form-table').click(function(event) {
			$(this).mncf7ToggleFormTable();
		}).mncf7ToggleFormTable();
		...
		$('[data-config-field]').each(function() {
			...
			if (_mncf7.configErrors[section]) {
				var $list = $('<ul></ul>').attr({
					'role': 'alert',
					'class': 'config-error'
				});

				$.each(_mncf7.configErrors[section], function(i, val) {
		...
		$(window).on('beforeunload', function(event) {
			...
			$('#mncf7-admin-form-element :input[type!="hidden"]').each(function() {
			...
			if (changed) {
				event.returnValue = _mncf7.saveAlert;
				return _mncf7.saveAlert;
			}
		});

		$('#mncf7-admin-form-element').submit(function() {
	...
	$.fn.mncf7ToggleFormTable = function() {
	...
	/**
	 * Copied from mntitlehint() in admin/js/post.js
	 */
	$.mncf7TitleHint = function() {
		var title = $('#title');
